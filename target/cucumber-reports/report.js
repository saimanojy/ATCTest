$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("Features/ACTScenario.feature");
formatter.feature({
  "line": 1,
  "name": "ACT Scenario feature file",
  "description": "",
  "id": "act-scenario-feature-file",
  "keyword": "Feature"
});
formatter.scenarioOutline({
  "line": 2,
  "name": "ACT Round2 Scenario",
  "description": "",
  "id": "act-scenario-feature-file;act-round2-scenario",
  "type": "scenario_outline",
  "keyword": "Scenario Outline"
});
formatter.step({
  "line": 3,
  "name": "valid registered email and password in \u003crow_number\u003e row",
  "keyword": "Given "
});
formatter.step({
  "line": 4,
  "name": "clicked on SignIn",
  "keyword": "And "
});
formatter.step({
  "line": 5,
  "name": "login should be successfull",
  "keyword": "Then "
});
formatter.step({
  "line": 6,
  "name": "clicked My addresses",
  "keyword": "When "
});
formatter.step({
  "line": 7,
  "name": "addresses page should be navigated successfully",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "clicked Add New Addresses",
  "keyword": "When "
});
formatter.step({
  "line": 9,
  "name": "Add address pages should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user fills all fields from \u003crow_number\u003e with new data and click on save",
  "keyword": "When "
});
formatter.step({
  "line": 11,
  "name": "my address page should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "clicked on women and selected summer dresses",
  "keyword": "When "
});
formatter.step({
  "line": 13,
  "name": "summer dresses page should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 14,
  "name": "change view to list view",
  "keyword": "Then "
});
formatter.step({
  "line": 15,
  "name": "add 3 items to cart",
  "keyword": "And "
});
formatter.step({
  "line": 16,
  "name": "clicked on checkout",
  "keyword": "When "
});
formatter.step({
  "line": 17,
  "name": "order page should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 18,
  "name": "complete the order",
  "keyword": "Then "
});
formatter.step({
  "line": 19,
  "name": "clicked on username",
  "keyword": "When "
});
formatter.step({
  "line": 20,
  "name": "acoount details should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 21,
  "name": "clicked order history and details",
  "keyword": "When "
});
formatter.step({
  "line": 22,
  "name": "order history should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 23,
  "name": "clicked sign out",
  "keyword": "When "
});
formatter.step({
  "line": 24,
  "name": "signIn home page should be displayed",
  "keyword": "Then "
});
formatter.examples({
  "line": 26,
  "name": "",
  "description": "",
  "id": "act-scenario-feature-file;act-round2-scenario;",
  "rows": [
    {
      "cells": [
        "row_number"
      ],
      "line": 27,
      "id": "act-scenario-feature-file;act-round2-scenario;;1"
    },
    {
      "cells": [
        "1"
      ],
      "line": 28,
      "id": "act-scenario-feature-file;act-round2-scenario;;2"
    }
  ],
  "keyword": "Examples"
});
formatter.scenario({
  "line": 28,
  "name": "ACT Round2 Scenario",
  "description": "",
  "id": "act-scenario-feature-file;act-round2-scenario;;2",
  "type": "scenario",
  "keyword": "Scenario Outline"
});
formatter.step({
  "line": 3,
  "name": "valid registered email and password in 1 row",
  "matchedColumns": [
    0
  ],
  "keyword": "Given "
});
formatter.step({
  "line": 4,
  "name": "clicked on SignIn",
  "keyword": "And "
});
formatter.step({
  "line": 5,
  "name": "login should be successfull",
  "keyword": "Then "
});
formatter.step({
  "line": 6,
  "name": "clicked My addresses",
  "keyword": "When "
});
formatter.step({
  "line": 7,
  "name": "addresses page should be navigated successfully",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "clicked Add New Addresses",
  "keyword": "When "
});
formatter.step({
  "line": 9,
  "name": "Add address pages should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user fills all fields from 1 with new data and click on save",
  "matchedColumns": [
    0
  ],
  "keyword": "When "
});
formatter.step({
  "line": 11,
  "name": "my address page should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "clicked on women and selected summer dresses",
  "keyword": "When "
});
formatter.step({
  "line": 13,
  "name": "summer dresses page should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 14,
  "name": "change view to list view",
  "keyword": "Then "
});
formatter.step({
  "line": 15,
  "name": "add 3 items to cart",
  "keyword": "And "
});
formatter.step({
  "line": 16,
  "name": "clicked on checkout",
  "keyword": "When "
});
formatter.step({
  "line": 17,
  "name": "order page should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 18,
  "name": "complete the order",
  "keyword": "Then "
});
formatter.step({
  "line": 19,
  "name": "clicked on username",
  "keyword": "When "
});
formatter.step({
  "line": 20,
  "name": "acoount details should be opened",
  "keyword": "Then "
});
formatter.step({
  "line": 21,
  "name": "clicked order history and details",
  "keyword": "When "
});
formatter.step({
  "line": 22,
  "name": "order history should be displayed",
  "keyword": "Then "
});
formatter.step({
  "line": 23,
  "name": "clicked sign out",
  "keyword": "When "
});
formatter.step({
  "line": 24,
  "name": "signIn home page should be displayed",
  "keyword": "Then "
});
formatter.match({
  "arguments": [
    {
      "val": "1",
      "offset": 39
    }
  ],
  "location": "StepDefinitions.valid_registered_email_and_password_in_row(int)"
});
formatter.result({
  "duration": 24626281600,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_on_SignIn()"
});
formatter.result({
  "duration": 2065336900,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.login_should_be_successfull()"
});
formatter.result({
  "duration": 2447099800,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_My_addresses()"
});
formatter.result({
  "duration": 1561647500,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.addresses_page_should_be_navigated_successfully()"
});
formatter.result({
  "duration": 24324500,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_Add_New_Addresses()"
});
formatter.result({
  "duration": 1381173100,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.add_address_pages_should_be_displayed()"
});
formatter.result({
  "duration": 8111100,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "1",
      "offset": 27
    }
  ],
  "location": "StepDefinitions.user_fills_all_fields_from_with_new_data_and_click_on_save(int)"
});
formatter.result({
  "duration": 5842775100,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.my_address_page_should_be_displayed()"
});
formatter.result({
  "duration": 2906159700,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_on_women_and_selected_summer_dresses()"
});
formatter.result({
  "duration": 2333339200,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.summer_dresses_page_should_be_opened()"
});
formatter.result({
  "duration": 5173096100,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.change_view_to_list_view()"
});
formatter.result({
  "duration": 128589000,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "3",
      "offset": 4
    }
  ],
  "location": "StepDefinitions.add_items_to_cart(int)"
});
formatter.result({
  "duration": 31404790000,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_on_checkout()"
});
formatter.result({
  "duration": 3243316900,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.order_page_should_be_opened()"
});
formatter.result({
  "duration": 16706400,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.complete_the_order()"
});
formatter.result({
  "duration": 10103611400,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_on_username()"
});
formatter.result({
  "duration": 582232500,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.acoount_details_should_be_opened()"
});
formatter.result({
  "duration": 1251064900,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_order_history_and_details()"
});
formatter.result({
  "duration": 1040720100,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.order_history_should_be_displayed()"
});
formatter.result({
  "duration": 3459017100,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.clicked_sign_out()"
});
formatter.result({
  "duration": 2391268700,
  "status": "passed"
});
formatter.match({
  "location": "StepDefinitions.signin_home_page_should_be_displayed()"
});
formatter.result({
  "duration": 8823600,
  "status": "passed"
});
formatter.after({
  "duration": 2058337600,
  "status": "passed"
});
formatter.after({
  "duration": 248200,
  "status": "passed"
});
});